...
function projectDetails(pProjectKey, projectName, projectOpenIndex) {
	...
	$.ajax({
		url: 'https://jira.codeswat.com/browse/' + pProjectKey + '-1?jql=project%20%3D%20"' + pProjectKey + '"',
		method: 'GET',
		headers: {
			'Cookie': params
		},
		success: function(data) {
			...
			$.each(resultJSON.issueTable.issueKeys, function(index, issueKey) {
				promises.push($.ajax({
					url: 'https://jira.codeswat.com/browse/' + issueKey + '?page=com.atlassian.jira.plugin.system.issuetabpanels:worklog-tabpanel',
					method: 'GET',
					headers: {
						'Cookie': params
					},
