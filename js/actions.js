...
function getTasks(pProjectKey, pCookieParams, pPromises) {
	...
	$.each(statuses, function(index, value) {
		promises.push(
			$.ajax({
				url: 'https://jira.codeswat.com/browse/' + pProjectKey + '-1?jql=project%20%3D%20"' + pProjectKey + '"%20AND%20status%20%3D%20' + value.urlValue,
				method: 'GET',
				headers: {
...
function getUsers() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			var endIndex = data.lastIndexOf('<footer id="footer" role="contentinfo">');
			var projectsCodeSwat = '<div><div><div><div><table>' + data.substring(startIndex, endIndex);
			var projectsArray = $(projectsCodeSwat).find('.projects-list').find('tr');
			var countProjects = projectsArray.length;			
			var promises = [];
			var usersLinks = [];

			$.each(projectsArray, function(index, value) {
				var projectKey = $.trim($(value).find('[data-cell-type="name"]').next().text());

				promises.push($.ajax({
					url: 'https://jira.codeswat.com/browse/' + projectKey + '-1?filter=-1&jql=project%20%3D%20"' + projectKey + '"%20AND%20updated%20>%3D%20-2w',
					method: 'GET',
					headers: {
						'Cookie': params
					},
							...
							promises.push($.ajax({
								...
								success: function(issuePage) {
									...
									$.each(workLogs, function(index, workLog) {
										var nickNameWokrlog = $(workLog).find('.action-details').find('a.user-avatar').attr('rel');
										var userNameWorklog = $.trim($(workLog).find('.action-details').find('a.user-avatar').text());
										...
										if ((nickNameWokrlog != nickName) && (typeof(nickNameWokrlog) != "undefined") && isStatisticIssues(issueDate)) {
											addUser(nickNameWokrlog, value, userNameWorklog);
										}
									});
								},
								fail: function(data) {
...
function userDetail(pNickName, pUserOpenIndex) {
	...
	$.each(users.get(pNickName).issues, function(index, issueName) {
		promises.push($.ajax({
			url: 'https://jira.codeswat.com/browse/' + issueName + '?page=com.atlassian.jira.plugin.system.issuetabpanels:worklog-tabpanel',
			method: 'GET',
			headers: {
				'Cookie': params
			},
