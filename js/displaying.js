...
function displayUserProgress(pUsers, pCountProjects) {
	currentProjectNumber++;
	var countAllIssues = 0;
	for (var nickName of pUsers.keys()) {
		var countAllIssues = pUsers.get(nickName).open + pUsers.get(nickName).inprogress + pUsers.get(nickName).resolved;
	    var openProgress = parseInt((pUsers.get(nickName).open / countAllIssues) * 100);
		var inProgress = parseInt((pUsers.get(nickName).inprogress / countAllIssues) * 100);
		var closedProgress = parseInt(100 - openProgress - inProgress);

		var taskOpenLabel = (pUsers.get(nickName).open == 1) ? 'task' : 'tasks';
		var taskInProgressLabel = (pUsers.get(nickName).inprogress == 1) ? 'task' : 'tasks';
		var taskClosedLabel = (pUsers.get(nickName).resolved == 1) ? 'task' : 'tasks';
		...
		if (currentProjectNumber == pCountProjects) {
			(function(nickName) {
		...
		var loadingStr = ' Loading users\' projects: ' + currentProjectNumber + ' of ' + pCountProjects;
		loadingStr = (currentProjectNumber != pCountProjects) ? img + loadingStr : loadingStr; 
